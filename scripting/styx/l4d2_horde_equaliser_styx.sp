...
public OnMapStart()
{
	mapcommonLimit = L4D2_GetMapValueInt("horde_limit", -1);
...
{
	commonTotal = 0;
	int muit = (GetConVarInt(FindConVar("survivor_limit"))+1)/2;
	commonLimit = mapcommonLimit * muit / 2;
	Horde_min_size = HORDE_MIN_SIZE_AUDIAL_FEEDBACK * muit / 2;
	lastCheckpoint = 0;
	announcedInChat = false;
	for (new i = MAX_CHECKPOINTS; i > 0; i--)
	{
		checkpointAnnounced[i-1] = false;
	}
}
...
public OnEntityCreated(entity, const String:classname[])
{
	...
	if (StrEqual(classname, "infected", false) && IsInfiniteHordeActive())
	{
		...
		{
			if (commonLimit >= Horde_min_size) EmitSoundToAll(HORDE_SOUND);
